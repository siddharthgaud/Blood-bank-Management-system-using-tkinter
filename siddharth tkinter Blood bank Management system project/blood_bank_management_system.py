...
def adddonar():
    ...
    def add():
      ...
      #  current_date=datetime.datetime.now().date()
       ...
       if(phone.isdigit()==TRUE and len(phone)<10):
        ...
        conn = mysql.connect(host="localhost",user="root",password="Siddharth@1996",database="blood_bank")
        cursor = conn.cursor()
        

        cursor.execute("insert into reco values('"+e1.get()+"','"+e2.get()+"','"+e3.get()+"','"+e4.get()+"','"+e5.get('1.0',END)+"','"+e6.get()+"','"+e7.get()+"')")
        cursor.execute("commit")

        cursor.execute("update bld_stock set NO_OF_PACKETS=NO_OF_PACKETS+1 where BLOOD_GROUP='"+e4.get()+"'")
        cursor.execute("commit")
        conn.close()
    ...
    w2.configure(bg="light coral")
    w2.resizable(0,0)
    w2.title("DONAR FORM")

    Label(w2,text="FILL DONAR FORM",bg="light coral").pack()
    l1=Label(w2,text="FULL_NAME:",bg="light coral",font=("times new roman",10))
    l1.place(x=30,y=50)
    e1=Entry(w2)
    e1.focus()
    e1.place(x=130,y=50)

    l2=Label(w2,text="AGE:",bg="light coral",font=("times new roman",10))
    l2.place(x=30,y=80)
    e2=Entry(w2)
    e2.place(x=130,y=80)
    ...
    l=Label(w2,text="SEX:",bg="light coral",font=("times new roman",10))
    l.place(x=30,y=110)
    ...
    l3=Label(w2,text="BLOOD_GROUP:",bg="light coral",font=("times new roman",10))
    l3.place(x=30,y=140)
    ...
    l4=Label(w2,text="ADDRESS:",bg="light coral",font=("times new roman",10))
    l4.place(x=30,y=170)
    e5=Text(w2,height=2,width=16)
    e5.place(x=130,y=170)
                        
    l5=Label(w2,text="CONTACT_NO:",bg="light coral",font=("times new roman",10))
    l5.place(x=30,y=220)
    e6=Entry(w2)
    e6.place(x=130,y=220)
                        
    l6=Label(w2,text="DATE:",bg="light coral",font=("times new roman",10))
    l6.place(x=30,y=250)
    e7=Entry(w2)
    e7.place(x=130,y=250)
    sub=Button(w2,text="SUBMIT",bg="green",fg="white",width=10,font=("times new roman",10),command=add)
    sub.place(x=110,y=280)
    res=Button(w2,text="RESET",bg="green",fg="white",width=10,font=("times new roman",10),command=crl)
...
def fetch():
    ...
    conn=mysql.connect(host="localhost",user="root",password="Siddharth@1996",database="blood_bank")
    cursor=conn.cursor()
    cursor.execute("select * from reco order by blood_group asc")
    ...
    r=Tk()
    r.geometry("600x500+100+50")
        
    r.title("DONAR RECORDS")
    r.configure(bg="light coral")
        
    l1=ttk.Treeview(r,columns=(1,2,3,4,5,6,7),show="headings",selectmode='browse')
    l1.grid()
...
def fetch1():
    conn=mysql.connect(host="localhost",user="root",password="Siddharth@1996",database="blood_bank")
    cursor=conn.cursor()
    cursor.execute("select * from bld_stock")
    ...
    r1=Tk()
    r1.configure(bg="light coral")
    r1.geometry("500x500+100+50")
    r1.title("BLOOD STOCK")
    r1.resizable(0,0)
        
    l1=ttk.Treeview(r1,columns=(1,2),show="headings")
    l1.pack()
    l1.heading(1,text="BLOOD_GROUP")
    l1.heading(2,text="NO_OF_PACKETS")
...
def fetch3():
    ...
    def srch(): 
        conn=mysql.connect(host="localhost",user="root",password="Siddharth@1996",database="blood_bank")
        cursor=conn.cursor()
        cursor.execute("select * from reco where blood_group='"+e11.get()+"' order by blood_group asc" )
...
c=Canvas(root,bg="blue",height=300,width=700)

img4 = ImageTk.PhotoImage(Image.open(r"C:\\Users\SIDDHARTH\Desktop\\tkinter\3.jpg"))
